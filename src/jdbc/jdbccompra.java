...
public class jdbccompra {
	...
	public List<Compra> listarTodo(){
		...
		try {
			...
			String sql="SELECT * FROM vivanda.compras INNER JOIN vivanda.proveedores ON compras.cod_proveedor = proveedores.cod_proveedor";			
			...
			while(rs.next()){
				...
				compra = new Compra(pro,rs.getDouble("precio_total"),detalle,rs.getInt("cod_compra"));				
				compras.add(compra);
			}			
	...
	public List<Compra> buscar(String nom_proveedor){
		...
		try {					
			Connection cn = c.getConnection();
			String sql="SELECT * FROM vivanda.compras INNER JOIN vivanda.proveedores ON compras.cod_proveedor = proveedores.cod_proveedor where LOWER(proveedores.razonsocial) like ?";
			PreparedStatement ps =(PreparedStatement) cn.prepareStatement(sql);
	...
	public void editar(Compra compra){
		try {			
			Connection cn = c.getConnection();
			String sql_compra="Update compras set cod_proveedor = ? , precio_total = ?  where cod_compra = ?";
			...
			ps_compra.setInt(3, compra.getId());
	...
	public void registrar(Compra compra){
		try {			
			Connection cn = c.getConnection();
			String sql_compra="INSERT INTO compras (precio_total,cod_proveedor) VALUES (?,?);";
	...
	public void registrarDetalleCompra(DetalleCompra detalle){
		try {			
			Connection cn = c.getConnection();
			String sql_detalle = "inser into detalle_compra(cod_compra,cod_articulo,precio_unitario,cantidad) values(?,?,?,?)";
			PreparedStatement ps_detalle=(PreparedStatement) cn.prepareStatement(sql_detalle);
